...
function useSkill_after_Priest(obj, skillIndex, consumeMp, consumeItem, oldSkillMpRate)
{
	...
	if(skillLevel > 0)
	{
		...
		if(skillIndex == 47)
		{
			...
			local appendage = CNSquirrelAppendage.sq_AppendAppendage(obj, obj, 47, false, "character/xinghe/priest/shishenzhili/ap_mailun_lieyan.nut", false);
		...
		else if(skillIndex == 48)
		{
			...
			local appendage = CNSquirrelAppendage.sq_AppendAppendage(obj, obj, 48, false, "character/xinghe/priest/shishenzhili/ap_mailun_shengguang.nut", false);
			appendage.sq_SetValidTime(duration);
			CNSquirrelAppendage.sq_Append(appendage, obj, obj);
		}
	}
