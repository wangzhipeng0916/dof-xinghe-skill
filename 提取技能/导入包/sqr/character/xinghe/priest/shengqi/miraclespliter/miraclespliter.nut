...
function onSetState_priest_miraclespliter(skillUser, stateData, skillData, customData)
{
    ...
    {
        case 0:
            if (CNSquirrelAppendage.sq_IsAppendAppendage(skillUser, "character/xinghe/priest/shengqi/jupiter/ap_jupiter.nut") == true)
    ...
    if (subState == 0)
    {
            ...
            && !sq_IsFixture(target))
        {
            if (!attacker.getVar().is_obj_vector(target))
            {
                ...
                if (CNSquirrelAppendage.sq_IsAppendAppendage(target, "character/xinghe/priest/shengqi/miraclespliter/ap_miraclespliter.nut"))
                    CNSquirrelAppendage.sq_RemoveAppendage(target, "character/xinghe/priest/shengqi/miraclespliter/ap_miraclespliter.nut");
                
                CNSquirrelAppendage.sq_AppendAppendage(target, attacker, 247, true, "character/xinghe/priest/shengqi/miraclespliter/ap_miraclespliter.nut", true);
            }
        }
    }
};
