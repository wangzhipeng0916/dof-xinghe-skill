...
function onAttack_priest_devilfist(attacker, target, attackInfo, isCounterAttack)
{
    ...
    if(sq_IsGrabable(attacker, target)
        ...
        && !sq_IsFixture(target)
        && !CNSquirrelAppendage.sq_IsAppendAppendage(target, "character/xinghe/priest/fuchou/devilfist/ap_devilfist.nut"))
    {
        local distance = attacker.getXDistance(target);
        if(distance < 160) distance = 160;
        local appendage = CNSquirrelAppendage.sq_AppendAppendage(target, attacker, SKILL_PRIEST_DEVILFIST, false, "character/xinghe/priest/fuchou/devilfist/ap_devilfist.nut", true);
